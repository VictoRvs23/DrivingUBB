    ...
    const handleFinalizarEvaluacion=async(e)=>{
        ...
        try{
            ...
            if (puntajeCalculado < 60) {
                notaChilena = 3 * (puntajeCalculado / 60) + 1;
            } else {
                notaChilena = 3 * ((puntajeCalculado - 60) / 40) + 4;
            }
            
            const notaFinal = parseFloat(notaChilena.toFixed(1));
                    
            let obs = '';
            if(faltas.length === 0) {
                obs = `Evaluación perfecta. Sin faltas registradas. Nota: ${notaFinal}`;
            } else if(tieneFaltaCritica) {
                obs = `Evaluación reprobada por falta crítica. Total faltas: ${faltas.length}. Nota: ${notaFinal}`;
            } else {
                obs = `Evaluación completada. Total faltas: ${faltas.length}. Nota: ${notaFinal}`;
            }

            await finalizarEvaluacionRequest(evaluacionId, {
                puntaje_obtenido: puntajeCalculado, 
                nota_final: notaFinal,
                observaciones: obs
            });
             setPuntajeObtenido(notaFinal);
        ...
        <div className="evaluacion-practica-container">
            ...
            {step === 2 && (
                <div className="evaluacion-faltas">
                    <h2>Registrar Faltas - Evaluación #{evaluacionId}</h2>

                    {faltaCritica && (
                        <div className="warning-message">
                            FALTA CRÍTICA DETECTADA - Evaluación reprobada automáticamente. No se pueden registrar más faltas, finalice la evaluación.
                        </div>
                    )}
                ...
                <div className="evaluacion-resultados">
                    ...
                    <div className="resultado-header">
                        <div className="score-badge-eval" style={{
                            background: puntajeObtenido >= 4.0 
                                ? 'linear-gradient(135deg, #d4edda 0%, #c3e6cb 100%)' 
                                : 'linear-gradient(135deg, #f8d7da 0%, #f5c6cb 100%)',
                            border: puntajeObtenido >= 4.0 ? '3px solid #27ae60' : '3px solid #e74c3c'
                        }}>
                            <span className="score-value-eval">{puntajeObtenido.toFixed(1)}</span>
                            <span className="score-label-eval">
                                {puntajeObtenido >= 4.0 ? 'APROBADO' : 'REPROBADO'}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="resultado-stats">
                        <div className="stat-box">
                            ...
                            <span className={`stat-value ${puntajeObtenido >= 4.0 ? 'green' : 'red'}`}>
                                {puntajeObtenido.toFixed(1)}
                            </span>
                        </div>
                        <div className="stat-box">
